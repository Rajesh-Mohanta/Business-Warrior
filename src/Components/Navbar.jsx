    ...
    <nav className="relative flex items-center justify-between py-4 px-4 md:px-6 lg:px-8 bg-white max-w-[1200px] mx-auto">
        ...
        <ul className="flex list-none gap-6 lg:gap-8 items-center">
          {navLinks.map((nav, index) => (
            ...
            >
              <a
                href={`#${nav.href}`}
                className={`flex items-center transition-colors duration-300 text-black hover:text-sky-500 text-sm md:text-base lg:text-lg cursor-pointer`}
              >
                {nav.label}
      ...
      {mobileMenuVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-10">
          <div className="absolute top-0 right-0 w-64 bg-white border border-gray-200 rounded-lg shadow-lg py-2 mt-16 z-20">
            <ul>
              {navLinks.map((nav, index) => (
                <li key={index} className="relative">
                  <a
                    ...
                    onClick={() => {
                      handleMobileDropdownToggle(nav.label);
                    }}
                  >
                    {nav.label}
                    {(nav.label === "Payplan" || nav.label === "Services") && (
